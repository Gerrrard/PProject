...
public class BoardNew : MonoBehaviour
{
            ...
            if (lv == 1)
            {
                if (col == 1 || col == 3)
                {
                    if ((x + 1) < 10 && Blocks[x + 1, y])
                    {
                        ...
                        {
                            Sphere S = gameObject.AddComponent<Sphere>();
                            S.x = x + 1;
                            S.y = y;
                            LisSpheres.Add(S);
                        }
                ...
                if (hit.transform != null)
                {
                    ...
                    if (Caught.TryGetComponent<Piece>(out PieC))
                    {
                        ...
                        else if (turn.PieceList.Contains(PieC))
                        {
                            HideSpheres(turn, null);
                            SelPiece = PieC;
                            PossibleMoves(false, SelPiece.x,SelPiece.y);
                    ...
                    else if (Caught.TryGetComponent<Sphere>(out SphC))
                    {
                        ...
                        MovePiece(SelPiece, SphC.x, SphC.y);
    ...
    {
        int CC = LisSpheres.Count;
        int k = -1;
        if (S != null)  //not selection
        {
            k = LisSpheres.IndexOf(S);
            stepID = SpheresIntoId(S);
            S.Selected();
        }
        for (int i = 0; i < CC; i++)
        {
            LisSpheres[i].Disappear();
            Destroy(LisSpheres[i].gameObject);
            LisSpheres.RemoveAt(i);
        }
    }   //READY

    public void ShowSpheres(Player pl)
    {
        for (int i = 0; i < LisSpheres.Count; i++)
        {
            GameObject SpH = Instantiate(sphere) as GameObject;
            int Xtemp = LisSpheres[i].x;
            int YTemp = LisSpheres[i].y;
            List<Piece> TempToBeKilled = LisSpheres[i].ToBeKilled;
            LisSpheres[i] = SpH.GetComponent<Sphere>();
            LisSpheres[i].x = Xtemp;
            LisSpheres[i].y = YTemp;
            LisSpheres[i].ToBeKilled = TempToBeKilled;
            SpH.transform.position = new Vector3(-4.5f + LisSpheres[i].x, 0.7f, -4.5f + LisSpheres[i].y);
            //LisSpheres[i].Appear();
        }
    }   //READY
    ...
    {
        //Debug.Log(P.transform. +  "    " + P.transform.position.y);
        pieces[P.x, P.y] = null;
        pieces[x, y] = P;
        P.x = x;
        P.y = y;
